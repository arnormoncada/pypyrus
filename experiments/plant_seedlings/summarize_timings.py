...
def main() -> int:
    ...
    pair_count = min(len(with_inst), len(without_inst))
    if pair_count > 0:
        paired_with = with_inst[:pair_count]
        paired_without = without_inst[:pair_count]
        ...
        print("\nPaired Deltas (with_i - without_i)")
        ...
        for idx, (w, wo) in enumerate(zip(paired_with, paired_without), start=1):
            ...
            print(
                f"  pair {idx:2d}: "
                f"with={format_seconds(w)} "
                f"without={format_seconds(wo)} "
                f"delta={delta_seconds:+.6f}s "
                f"pct={pct_text}"
            )
    elif with_inst or without_inst:
        print("\nPaired Deltas (with_i - without_i)")
        print("  not available: both conditions need at least one run")
